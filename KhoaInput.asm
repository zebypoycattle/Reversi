...
inputingMove:
	...
	la	$a0, inputCoord		# read the string of input (2 characters + null-terminate)
	li	$a1, 3
	li	$v0, 8
	syscall
	...
	la	$t1, inputCoord
	addi	$t1, $t1, 1
	lbu	$t0, ($t1)			# load byte, address offset by 1 byte starting at inputCoord data address
	blt	$t0, 65, invalidCoord		# second coord is < A, invalid
	blt	$t0, 73, digitUpperAlpha	# second coord is upper letter <= H, valid
	blt	$t0, 97, invalidCoord		# second coord was not upper letter <= H, is < a, invalid
